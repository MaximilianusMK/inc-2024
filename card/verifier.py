...
def main(argv):
  ...
  def check(_, list_name, limit):
    D = vr.ReadInt(1, limit)
    vr.ReadEndl()

    w_set = set()
    sum_of_length_of_w = 0
    for i in range(D):
      W = vr.ReadString(string.ascii_uppercase, 1, limit)
      vr.ReadEndl()
      w_set.add(W)
      sum_of_length_of_w += len(W)
    assert sum_of_length_of_w <= limit, ("sum of length of %s is bigger than %d" % list_name, limit)
    assert len(w_set) == D, ("The set of strings %s is not unique" % list_name)
